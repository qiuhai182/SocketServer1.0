...
int wait_client(int listen_socket, const char *targetServerIP, unsigned int server_port);
...
int wait_client(int listen_socket, const char *targetServerIP, unsigned int server_port)
{
	...
	for (;;)
	{
		...
		if (pollFd.revents & POLLRDNORM)
		{
			...
			pSockPair->m_svr_socket = connect_server(targetServerIP, server_port);
...
int main(int argc, const char **argv)
{
	const char *targetServerIP = "192.168.137.1"; // 待转发服务所在ip
	unsigned int server_port = 8888;		 // 待转发服务所在端口
	if (argc >= 3)
	{
	...
	wait_client(listen_socket, targetServerIP, server_port);
